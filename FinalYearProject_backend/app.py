...
ngrokroute="http://eb118555d51c.ngrok.io"
...
def download_qrcode():
	...
	if request.method=='POST':
		try:
			...
			qr=pyqrcode.create(PRODUCT_INFO_LINK+str(upc))
			name=str(upc)+'.png'
...
def manufacturer_add():
	...
	if request.method=='POST':
		try:
			...
			print(PRODUCT_INFO_LINK+str(upc))
			qr=pyqrcode.create(PRODUCT_INFO_LINK+str(upc))			
			# print(1)
			name=str(upc)+'.png'
			file_to_open=dir_path/name
			...
			tx_hash=contract.functions.manufactureItem(upc,itemType,itemName,productPrice,timestamp,location,productQuantity,productExpiryDate,productBatch).transact({"from":session['logged_in']['address']})
			tx_receipt = web3.eth.waitForTransactionReceipt(tx_hash)
		...
		try:
			...
			for item in sale:
				...
				if(item_info['Entry State']==2 and credentials[keys['0x'+item_info['By']]]['usertype']=='Manufacturer'):
					itemDetails[3] = web3.fromWei(itemDetails[3],'ether')
...
def product_info():
	...
	if request.method=='POST':
		try:
			productitems=[]
			upc=int(request.form['upc'])
			tp_temp = contract.functions.fetchProductDetails(upc).call()
			tp_temp[3] = web3.fromWei(tp_temp[3],'ether')
			productitems.append(tp_temp)
	...
	if request.method=='POST':
		try:
			...
			for i in range(1,index+1):
				...
				temp_item_detail = contract.functions.fetchItemHistoryArray(upc,i).call()
				for j in credentials:
					print(credentials[j])
					if(credentials[j]['address'] == temp_item_detail[2]):
						name = credentials[j]['name']
				temp_item_detail[2] = name
				temp_item_detail[3] = web3.fromWei(temp_item_detail[3],'ether')
				productitems.append(temp_item_detail)
...
def home_product_info():
	...
	if request.method=='POST':
		try:
			productitems=[]
			upc=int(request.form['upc'])
			new_temp = contract.functions.fetchProductDetails(upc).call()
			new_temp[3] = web3.fromWei(new_temp[3],'ether')
			productitems.append(new_temp)
...
@cross_origin
@app.route('/product_info/<identifier>', methods=['GET','POST'])
def c_product_info(identifier):
	try:
		productitems=[]
		upc=int(identifier)
		itemInfo=contract.functions.fetchProductDetails(upc).call()
		itemInfo[3] = web3.fromWei(itemInfo[3],'ether')
		try:
			approvaldata=requests.get(DRUG_INFO_LINK+itemInfo[2].replace('/','%20F')).json()['data']
			print(approvaldata)
		except:
